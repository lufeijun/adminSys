...
class SessionGuard implements StatefulGuard, SupportsBasicAuth
{
    ...
    protected function dealZdySession()
    {
        $user = \Auth::user();
        $data = [];
        $roleIds = explode(',', $user->role);
        $data['roles'] = \App\Model\Privilege\Role::whereIn('id', $roleIds)->pluck('name')->toArray();
        $data['action_granted'] = \App\Model\Privilege\Ability::whereIn('role_id',$roleIds)->where('type',2)->distinct()->pluck('ability')->toArray();
        // 处理菜单权限，
        $data['menu_first_granted'] = [];
        $data['menu_second_granted'] = [];
        $data['menu_three_granted'] = \App\Model\Privilege\Ability::whereIn('role_id',$roleIds)->where('type',1)->distinct()->pluck('ability','id')->toArray(); // 左侧二级
        $menu_first_granted = [];
        ...
        $this->session->put('administrator',$data);
        $this->dealFirstMenu();
    }

    private function dealFirstMenu()
    {
        $this->dealMenuOrder();
        $firstMenuGranted = getLoginedMessage('menu_first_granted',[]);
        $threeMenuGranted = getLoginedMessage('menu_three_granted',[]);
    ...
    private function dealMenuOrder()
     {
        $menus = config('menu.menu');
        $firstGranted = getLoginedMessage('menu_first_granted',[]);
